...
uint256 hashGenesisBlock("0x89c69b16bcc88bd581f450ce2f1dd327ce135d01c6cfca95ecd3c89a2957ae48");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Crocoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Crocoin Signed Message:\n";
...
{
    // Crocoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Crocoin: 840k blocks in ~4 years
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Crocoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Crocoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Crocoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 2116) { DiffMode = 2; } // crocoin, 100 blocks after first legacy diff adjustment
        }
        else {         
        	if (pindexLast->nHeight+1 >= 26754) { DiffMode = 2; }  //crocoin, 5 days after 27/01/2014 12:00 UTC
        }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Crocoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Crocoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[2] = 'o';
        pchMessageStart[3] = 'c';
        hashGenesisBlock = uint256("0xe3cfc8f0760739586e4cd534018b042fb95ff91be8dd704cd8a93b615a0fe5fb");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "05/11/2014 Umirovljenici organizirali milenijski skok u Savu kako bi spasili mirovinski sustav";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
//        block.nBits    = 0x1e0fffff;
        block.nNonce = 725597;
        block.nTime = 1399814606;

        if (fTestNet)
        {
            block.nNonce = 194925;
            block.nTime = 1399758236;
        }
...
//        printf("min nBit:  %08x\n", bnProofOfWorkLimit.GetCompact());
        ...
        assert(block.hashMerkleRoot == uint256("0x3eed1b1f1a70995fd405fdd5fa18a6ce0aeebc8d1b39d40f61e487d11351244f"));
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbf, 0xb5, 0xda }; // Crocoin: increase each by adding 1 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CrocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CrocoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CrocoinMiner(CWallet *pwallet)
{
    printf("CrocoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("crocoin-miner");
    ...
    try { loop {
        ...
        printf("Running CrocoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CrocoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CrocoinMiner, pwallet));
}
